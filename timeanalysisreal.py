...
class FieldAnalysis:
    ...
    def saveVTK3Dfft(self, varname):
        name = ("%s-t%#05.1f-fft3D.vtk" % (varname,self.tcenter))
    ...
    def saveKxOmega(self, varname):
        name = ("%s-t%#05.1f-kx-omega.txt" % (varname,self.tcenter))
        writeLengthNt = (self.grid.Nkt/2)
    ...
    def saveKxKyatomega(self, omegain, varname):
        ...
        for deltaifreq in range (-2,3,1):
            ...
            if(ifreqout>=0):
                ...
                name = ("%s-t%#05.1f-kx-ky-at-omega%4.2f.txt" % (varname,self.tcenter, omegaout))
                f1 = open(name, 'w')
    ...
    def saveNewData(self, timein, varname):
        ...
        name = ("%s-t%#05.1f-x-y.txt" % (varname,self.tcenter))
        f1 = open(name, 'w')
