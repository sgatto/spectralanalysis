...
class FieldAnalysis:
    ...
    def saveVTK3Dfft(self, varname):
        name = ("%s-t%#03f-fft3D.vtk" % (varname,self.tcenter))
    ...
    def saveKxOmega(self, varname):
        name = ("%s-t%#03f-kx-omega.txt" % (varname,self.tcenter))
        writeLengthNt = (self.grid.Nkt/2)
    ...
    def saveKxKyatomega(self, omegain, varname):
        ...
        for deltaifreq in range (-2,3,1):
            ...
            if(ifreqout>=0):
                ...
                name = ("%s-t%#03f-kx-ky-at-omega%4.2f.txt" % (varname,self.tcenter, omegaout))
                f1 = open(name, 'w')
